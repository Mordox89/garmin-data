...
def main():
    ...
    acts = api(f"/athlete/{ATHLETE}/activities",
               {"oldest": api_oldest, "newest": api_newest,
                "fields": "name,type,workout_type,start_date_local,distance,moving_time,elapsed_time,"
                          ...
                          "average_cadence,max_heartrate,icu_training_load"})
    ...
    # ---------- recentActivities: last 5 runs for AI feedback ----------
    def _pace_str(dist_m, secs):
        if not dist_m or not secs or dist_m <= 0:
            return None
        sec_km = secs / (dist_m / 1000.0)
        return f"{int(sec_km // 60)}:{int(sec_km % 60):02d}/km"

    recent_runs = sorted(
        [a for a in acts if a.get("type") in RUN_TYPES],
        key=lambda a: a.get("start_date_local", ""),
        reverse=True
    )[:5]

    OUT["recentActivities"] = []
    for _a in recent_runs:
        _dist  = _a.get("distance") or 0
        _t     = _a.get("moving_time") or _a.get("elapsed_time") or 0
        _hr    = _a.get("average_heartrate")
        _maxhr = _a.get("max_heartrate")
        _cad   = _a.get("average_cadence")
        _load  = _a.get("icu_training_load")
        _decouple = _a.get("decoupling")
        _zt    = _a.get("icu_hr_zone_times") or []
        _ztot  = sum(z or 0 for z in _zt)
        _zpct  = [round((z or 0) / _ztot * 100) for z in _zt[:5]] if _ztot > 0 else []
        OUT["recentActivities"].append({
            "date":          (_a.get("start_date_local") or "")[:10],
            "name":          _a.get("name", "Run"),
            "type":          _a.get("type"),
            "dist_km":       round(_dist / 1000.0, 2) if _dist else None,
            "moving_time_s": _t or None,
            "pace":          _pace_str(_dist, _t),
            "avg_hr":        round(_hr)    if _hr    else None,
            "max_hr":        round(_maxhr) if _maxhr else None,
            "cadence_spm":   round(_cad * 2) if _cad else None,
            "load":          round(_load)  if _load  else None,
            "decoupling":    round(_decouple, 1) if _decouple is not None else None,
            "zone_pct":      _zpct,
        })
